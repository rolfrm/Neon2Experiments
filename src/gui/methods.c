...
methods * methods_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"method_key", "method"};
  methods * instance = calloc(sizeof(methods), 1);
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(method_key), sizeof(method)}, (char *[]){(char *)"key", (char *)"value"});
...
bool methods_try_get(methods * table, method_key * key, method * value){
  ...
  unsigned int sizes[] = {sizeof(method_key), sizeof(method)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
