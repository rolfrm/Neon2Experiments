...
u64_to_u8 * u64_to_u8_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"u64", "u8"};
  u64_to_u8 * instance = calloc(sizeof(u64_to_u8), 1);
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(u64), sizeof(u8)}, (char *[]){(char *)"key", (char *)"value"});
...
bool u64_to_u8_try_get(u64_to_u8 * table, u64 * key, u8 * value){
  ...
  unsigned int sizes[] = {sizeof(u64), sizeof(u8)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
